...
function MainContent() {
  ...
  const handleAddToCart = useCallback(async (product, options = {}) => {
    try {
      ...
      if (product.id) {
        let prodData = null;
        try {
          prodData = await dataReader.getById("lacosteProducts", product.id);
        } catch (e) {
          console.warn("Could not fetch product from Firebase:", e);
        }
  ...
  const handleQtyChange = useCallback(async (item, delta) => {
    ...
    if (newQty < 1) {
      showError("الكمية لا يمكن أن تكون أقل من 1");
      return;
    }

    // Stock check when increasing quantity
    if (delta > 0 && item.originalProductId) {
      try {
        const prodData = products.find((p) => p.id === item.originalProductId);
        if (prodData) {
          const available = getAvailableQuantity(
            prodData,
            item.color || null,
          ...
          const existingInCart = cart
            .filter(
              (ci) =>
                ci.originalProductId === item.originalProductId &&
                (ci.color || "") === (item.color || "") &&
                (ci.size || "") === (item.size || "")
            )
            .reduce((sum, ci) => sum + (ci.quantity || 0), 0);

          if (newQty > available || existingInCart >= available) {
            const canAdd = available - existingInCart;
            showError(
    ...
    const result = await updateQuantity(item, delta);
    if (!result.success) {
      showError(result.message || "فشل تحديث الكمية");
    }
  }, [updateQuantity, cart, showError, products]);
        ...
        const stockChecks = await Promise.all(
          cart.map(async (item) => {
            ...
            try {
              let prodData = null;
              try {
                prodData = await dataReader.getById("lacosteProducts", item.originalProductId);
              } catch (e) {
                console.warn("Could not fetch product from Firebase:", e);
              }
  ...
  }, [cart, shop, clearCart, success, showError, warning]);
